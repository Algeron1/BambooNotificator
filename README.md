...
#### Новое
- **Карточки деплоя** теперь показывают, как был запущен деплой: вручную, по расписанию или автоматически (как часть цепочки).
- **Статус сборки** (билда) отображается прямо на карточке — успешная или упавшая.
- **Статус деплоя** вынесен в заголовок карточки в виде цветного бейджа.
- **Ветка и версия** выделены моноширинным шрифтом для лучшей читаемости.
- **Механизм баг-репортов**: авторизованные пользователи могут отправить сообщение об ошибке прямо из интерфейса. Репорты видны администратору в панели управления.
- **Просмотр логов приложения** в админ-панели — последние записи доступны по кнопке без перезагрузки страницы.

#### Исправления
- Устранены повторяющиеся запросы к Bamboo API для стендов, у которых нет связанного билда — результат теперь кэшируется.
- Исправлена ошибка парсинга кэша, из-за которой страница могла не загружаться после перезапуска.
